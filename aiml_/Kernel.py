...
class Kernel:
    ...
    _maxHistorySize = 80  # maximum length of the _inputs and _responses lists
    _maxRecursionDepth = 110  # maximum number of recursive <srai>/<sr> tags before the response is aborted.
    ...
    def _respond(self, inpt, sessionID):
        ...
        if len(inputStack) > self._maxRecursionDepth:
            if self._verboseMode:
                err = "WARNING: maximum recursion depth exceeded (input='%s')" % inpt.encode(self._textEncoding,
                                                                                             'replace')
                sys.stderr.write(err)
            return ""
